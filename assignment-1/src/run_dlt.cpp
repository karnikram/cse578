...
/* 	Image of world origin
	...
	Eigen::Vector4f X_origin;
	Eigen::Vector3f x_origin;
	X_origin << 0, 0, 0, 1;
	x_origin = calib.getPMatrix() * X_origin;
	x_origin = x_origin/x_origin(2);
	std::cout << "Image of world origin: " << x_origin << std::endl;
	cv::circle(frame,cv::Point(x_origin(0),x_origin(1)),30,cv::Scalar(0,255,0),-1,CV_AA);
 */
    ...
    cv::imwrite("../report/imgs/cube-overlay.png",frame);
	cv::waitKey(0);	
